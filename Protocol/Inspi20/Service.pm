...
sub init{
	...
	print $_sockID "CAPAB CAPABILITIES PROTOCOL=1202\r\n";
	...
	while (my $event = <$_sockID>) {
		...
		switch($args[1]) {
		        ...
		        case "QUIT" {
						...
						undef @{ $_userchan{$vuid} };
						if($_isauth{$vuid}) {
		        			&Core::setMemberUid("",$_Octopus->getLogin($vuid));
		        		}
		        }
		        case "FJOIN" {
		        		# :000 SJOIN #channel TIMESTAMP +chanmode :mode,UID		        		
		        		my $start = length($args[5]) - 10;
		        		my $end = length($args[5]);
		        		my $xvuid = substr $args[5], $start, $end;
		        		print "VUID : $xvuid\n";
		        		my $xvtarget = &Core::getNick($xvuid);
						#push @{ $_userchan{$xvtarget} }, $args[2];
						print "CHANNEL : $args[2]\n";
		        	   	if(&Core::checkClose($args[2])){
		        	   		print "WARN CLOSED\n";
							$_Octopus->join($args[2]);
        					$_Octopus->kick($args[2],$xvuid,"Closed channel !");	
		        		}
		        }                        
		        case "PART" {
