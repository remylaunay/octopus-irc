...
sub checkChan {
 my ( $chan ) = @_;
 my $sql = mySQL::connect;
 $chan = $sql->selectrow_array("SELECT id FROM chanlist WHERE chan = '".$chan."'");
 return $chan;
}
...
sub checkClose {
 my ( $chan ) = @_;
 my $sql = mySQL::connect;
 $chan = $sql->selectrow_array("SELECT id FROM closelist WHERE chan = '".$chan."'");
 return $chan;
}
...
sub checkNick {
 my ( $nick ) = @_;
 my $sql = mySQL::connect;
 $nick = $sql->selectrow_array("SELECT id FROM nicklist WHERE nick = '".$nick."'");
 return $nick;
}
