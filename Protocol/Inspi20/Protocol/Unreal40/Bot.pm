...
sub create {
	...
	print ":$this->{sid} UID $this->{nick} 1 ".time()." $this->{user} $this->{host} $this->{uid} 0 +BISqzxwos $this->{host} $this->{host} $this->{eaddr} :$this->{name}\r\n";	
...
sub kill {
 ...
 my $sockID = $this->{sockID};
 if(!defined($reason) || $reason eq "") {$reason = "Octopus";}
